...
int ff_log_printf(ff_log_level_t level, const char *tag, const char *fmt, ...)
{
	...
	/* Send to ff_device. */
	if (likely(g_context) && likely(g_config) &&
	    unlikely(g_config->logcat_driver)) {
		char *uevent_env[2] = { uevent_env_buf, NULL };
		kobject_uevent_env(&g_context->miscdev.this_device->kobj,
...
const char *ff_err_strerror(int err)
{
	...
	case FF_ERR_BUSY:
		return "Resource busy / Timeout";

	/* Common error. */
	case FF_ERR_BAD_PARAMS:
		return "Bad parameter(s)";
	case FF_ERR_NULL_PTR:
		return "Null pointer";
	case FF_ERR_BUF_OVERFLOW:
		return "Buffer overflow";
	case FF_ERR_BAD_PROTOCOL:
		return "Bad protocol";
	case FF_ERR_SENSOR_SIZE:
		return "Wrong sensor dimension";
	case FF_ERR_NULL_DEVICE:
		return "Device not found";
	case FF_ERR_DEAD_DEVICE:
		return "Device is dead";
	case FF_ERR_REACH_LIMIT:
		return "Up to the limit";
	case FF_ERR_REE_TEMPLATE:
		return "Template store in REE";
	case FF_ERR_NOT_TRUSTED:
		return "Untrusted enrollment";
...
static void ff_ctl_device_event(struct work_struct *ws)
{
	...
	kobject_uevent_env(&ctx->miscdev.this_device->kobj, KOBJ_CHANGE,
			   uevent_env);
...
static irqreturn_t ff_ctl_device_irq(int irq, void *dev_id)
{
	...
	if (likely(irq == ctx->irq_num)) {
		if (g_config && g_config->enable_fasync &&
		    g_context->async_queue) {
...
static int ff_ctl_report_key_event(struct input_dev *input,
...
{
	static char version[FF_DRV_VERSION_LEN] = {
		'\0',
	};
	FF_LOGV("'%s' enter.", __func__);
...
static int ff_ctl_fb_notifier_callback(struct notifier_block *nb,
				       unsigned long action, void *data)
{
	...
	uevent_env[1] = NULL;
	kobject_uevent_env(&g_context->miscdev.this_device->kobj, KOBJ_CHANGE,
			   uevent_env);
...
static int ff_ctl_register_input(void)
{
	...
	if (g_config) {
		input_set_capability(g_context->input, EV_KEY,
				     g_config->keycode_nav_left);
		input_set_capability(g_context->input, EV_KEY,
				     g_config->keycode_nav_right);
		input_set_capability(g_context->input, EV_KEY,
				     g_config->keycode_nav_up);
		input_set_capability(g_context->input, EV_KEY,
				     g_config->keycode_nav_down);
		input_set_capability(g_context->input, EV_KEY,
				     g_config->keycode_double_click);
		input_set_capability(g_context->input, EV_KEY,
				     g_config->keycode_click);
		input_set_capability(g_context->input, EV_KEY,
				     g_config->keycode_long_press);
		input_set_capability(g_context->input, EV_KEY,
				     g_config->keycode_simulation);
	}
...
#endif
int focal_detect_flag = 0;
/* Huaqin modify for ZQL1820 by puqirui at 2018/10/30 end*/
...
static int ff_ctl_free_driver(void)
{
	...
	if (g_context->irq_num > 0) {
		...
		if (err) {
			FF_LOGE("disable_irq_wake(%d) = %d.",
				g_context->irq_num, err);
		}
...
static int ff_ctl_init_driver(void)
{
	...
	do {
		...
		/* Register IRQ. */
		err = request_threaded_irq(g_context->irq_num,
					   ff_ctl_device_irq, NULL,
					   IRQF_TRIGGER_RISING, "ff_irq",
					   (void *)g_context);
		if (err) {
		...
		if (err) {
			FF_LOGE("enable_irq_wake(%d) = %d.", g_context->irq_num,
				err);
		}

		/* Register spidev device. For REE-Emulation solution only. */
		if (init_flag /*g_config && g_config->enable_spidev*/) {
#ifdef FF_SPI_SET
			err = ff_spi_init();
			if (err) {
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (g_log_level <= FF_LOG_LEVEL_DBG) {
		static const char *cmd_names[] = {
			...
			"FF_IOC_ENABLE_IRQ",
			"FF_IOC_DISABLE_IRQ",
			"FF_IOC_ENABLE_SPI_CLK",
			"FF_IOC_DISABLE_SPI_CLK",
			"FF_IOC_ENABLE_POWER",
			"FF_IOC_DISABLE_POWER",
			"FF_IOC_REPORT_KEY_EVENT",
			"FF_IOC_SYNC_CONFIG",
			"FF_IOC_GET_VERSION",
			"unknown",
		};
	...
	case FF_IOC_REPORT_KEY_EVENT: {
		ff_key_event_t kevent;
		if (copy_from_user(&kevent, (ff_key_event_t *)arg,
				   sizeof(ff_key_event_t))) {
	...
	case FF_IOC_SYNC_CONFIG: {
		if (copy_from_user(&driver_config, (ff_driver_config_t *)arg,
				   sizeof(ff_driver_config_t))) {
			FF_LOGE("copy_from_user(..) failed.");
			err = (-EFAULT);
			break;
		}
	...
	case FF_IOC_GET_VERSION: {
		if (copy_to_user((void *)arg, ff_ctl_get_version(),
				 FF_DRV_VERSION_LEN)) {
...
#ifdef CONFIG_COMPAT
static long ff_ctl_compat_ioctl(struct file *filp, unsigned int cmd,
				unsigned long arg)
{
	int err = 0;
