...
#if defined CONFIG_ARCH_MT6735M || defined CONFIG_ARCH_MT6755
#include <mt_spi.h>
...
#if 0 //defined (CONFIG_MTK_PLATFORM)
...
int ff_spi_init(void)
{
	...
	if (!g_master) {
		FF_LOGE("there is no spi master controller on bus #%d.",
			spi_desc.bus_num);
		return (-ENODEV);
	}
...
int ff_spi_write_buf(const void *tx_buf, int tx_len)
{
	...
	/* Verbose. */
	ff_log_printf(FF_LOG_LEVEL_DBG, LOG_TAG,
		      "ff_spi_write_buf(%p, %d):", xfer.tx_buf, xfer.len);
			       ...
			       int rx_len)
{
	...
	do {
		...
		/* Verbose TX buffer. */
		ff_log_printf(FF_LOG_LEVEL_DBG, LOG_TAG,
			      "ff_spi_WRITE_then_read_buf(%p, %d, ..):", tx_buf,
			      tx_len);
		...
		/* Verbose RX buffer. */
		ff_log_printf(FF_LOG_LEVEL_DBG, LOG_TAG,
			      "ff_spi_write_then_READ_buf(.., %p, %d):", rx_buf,
			      rx_len);
