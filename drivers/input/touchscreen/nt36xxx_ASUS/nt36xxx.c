...
const uint16_t gesture_key_array[] = {
	...
	KEY_TP_GESTURE_E,
	KEY_TP_GESTURE_S,
	KEY_TP_GESTURE_SWIPE_UP,
	KEY_TP_GESTURE_SWIPE_DOWN,
	KEY_TP_GESTURE_SWIPE_LEFT,
	KEY_TP_GESTURE_SWIPE_RIGHT,
};
                        ...
                        char __user *buffer, size_t size, loff_t *ppos)
{
	...
	return simple_read_from_buffer(buffer, size, ppos, ptr, (size_t)len);
}
                        ...
                        const char __user *buffer, size_t count, loff_t *off)
{
	...
	if (!bTouchIsAwake) {
		NVT_LOG("Touch is already sleep, unable to modify gesture node.\n");
		return count;
	}
...
nvt_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
